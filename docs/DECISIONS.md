...
## 2026-06-20 — Define popup: multi-line TRANSLATION parsing and viewport clamping
**Decision:** Two unrelated Define-popup bugs fixed together. (1) `parseDefineResponse()` (`js/prompt.js`) used `/^TRANSLATION:\s*(.+)$/m` — `.` doesn't match newlines, so when the model replied to a multi-sentence selection (e.g. a triple-clicked paragraph) with a TRANSLATION spanning more than one line, only the first line was captured, surfacing as "only the first sentence gets defined." Fixed by capturing `[\s\S]+` from `TRANSLATION:` to the end of the string (stripping a trailing ` ``` ` code-fence line if present), and by adding an explicit "write TRANSLATION as a single line, even for multi-sentence quotes" instruction to the system prompt as a belt-and-suspenders measure. (2) `#define-popup` is `position: fixed` and positioned at `selectionRect.bottom + 16`with no clamping; near the bottom of the viewport its height (highly variable — placeholder text, a multi-line translation, an optional base-form line, an optional Save-to-vocab button) could push it past `window.innerHeight`, truncating it with no way to recover it, since `html, body` have `overflow: hidden` — there is no document-level scroll for a fixed element to ever respond to, only the inner `#reading-panel` scrolls. Fixed with a new pure `clampPopupTop(top, height, viewportHeight, margin = 8)` helper in `js/display.js`, called from a `repositionDefinePopup()` wrapper in `js/app.js` after every DOM mutation that can change the popup's height (initial show, "No API key set", translation + base-form render, Save-button append, error message). `#define-popup` also gained `max-height: min(70vh, 420px); overflow-y: auto;` as a safety net for translations too long to fit even at the top of the viewport.  
**Rationale:** Both bugs were found via manual testing of the prior Define scope-creep fix (the multi-sentence-translation behavior the temperature/prompt fix made more likely to occur, since full-paragraph selections now get genuinely longer, more literal translations instead of a short padded guess). The parsing fix is the one guaranteed to work regardless of model compliance — the added prompt instruction is a cheap improvement but not load-bearing on its own, consistent with the project's existing stance (see the prior Define entries) that prompt instructions alone are not a reliable enforcement mechanism. For the popup, "scroll the page to reveal it" — the user's first framing of the symptom — can't work for a `position: fixed` element regardless of which container scrolls; repositioning the element itself is the only correct fix given the app's no-document-scroll layout. `clampPopupTop()` is extracted as a pure function (rather than inlined DOM math in `app.js`) specifically so it's unit-testable without a real layout engine — jsdom's `getBoundingClientRect()` always returns zeros, so the actual clamping arithmetic has to live somewhere that can be tested with hand-supplied numbers.

---

## 2026-06-19 — Define scope-creep fix: prompt constraint + per-call temperature (Option B)
**Decision:** Two changes, both scoped to the Define lookup path only. (1) `buildDefinePrompt()`'s system instruction (`js/prompt.js`) now states explicitly that `context` exists solely to disambiguate sense and is not text to translate, and that `TRANSLATION` must cover only the exact quoted span — never adding, inferring, or borrowing words from `context`, even when the quoted text reads as an incomplete fragment on its own. (2) `generateContent()` and its three provider functions (`callClaude`/`callOpenAI`/`callGoogle`) in `js/llm.js` gained an optional `temperature` parameter, spread into the request body only when defined (`...(temperature !== undefined && { temperature })`; Google nests it under `generationConfig`). The Define call site in `js/app.js` passes a new `DEFINE_TEMPERATURE = 0` constant; the story-generation call site passes no temperature argument, so it keeps each provider's existing default unchanged.  
**Rationale:** Investigation (see `docs/PLAN.md` "Define translation scope-creep") found two independent mechanisms behind inconsistent, padded Define translations for short selections like "pasado algunos": the system prompt never told the model that `context` was sense-disambiguation only, so the model filled in words from context to make the fragment read as a complete clause; and `generateContent()` had no determinism control, so repeated identical requests sampled differently each time. The two fixes are independent and were implemented together (Option B, chosen over prompt-only Option A) because the prompt fix alone does not make repeated lookups of the same selection consistent — variance was an explicit part of the reported symptom, not just the padding itself. Temperature stays opt-in via an optional parameter (default `undefined`, i.e. no change to the request body) specifically so story generation, which wants sampling variety, is unaffected. Near-zero rather than provider-specific "deterministic" modes was chosen as the simplest cross-provider lever; the investigation notes that even temperature 0 does not guarantee bit-identical output on every provider, and that genuinely ambiguous fragment selections spanning a verb-phrase boundary still have no single universally "correct" translation — this fix controls padding and variance, it does not resolve linguistic ambiguity that depends on grammar outside the selection.
